...
func (c *Client) Provision(ctx context.Context, name string, opts ProvisionOpts) error {
	...
	// rc.local only handles SSH bootstrap; egress and devtools are orchestrated
	// via zmx after SSH is ready.
	if opts.SSHPubKey != "" {
...
// rcLocalScript bootstraps SSH access on first boot. Egress and dev tools
// provisioning are handled separately via zmx after SSH is ready.
const rcLocalScript = `#!/bin/sh
set -e
